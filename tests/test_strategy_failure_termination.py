...
async def test_summarize_persistent_failure_terminates_partial(tmp_path):
    # 两个多描述实体,其中一个持续失败
    repo, data_root = _kb(tmp_path, entities=[
        {"title": "ACME", "type": "ORG", "description": ["d1", "d2"], "text_unit_ids": ["c1", "c2"], "frequency": 2},
        {"title": "BETA", "type": "ORG", "description": ["d3", "d4"], "text_unit_ids": ["c3", "c4"], "frequency": 2},
    ])
    step = repo.create_job(kb_id=1, type="full", specs=[StepSpec("summarize_descriptions", StepKind.UNIT_FANOUT)]).steps[0]
    worker = UnitWorker(repo=repo, adapter=_SummarizeFailAdapter("ACME"), data_root=data_root)
...
async def test_community_reports_persistent_failure_terminates_partial(tmp_path):
    repo, data_root = _kb(
        tmp_path,
        entities=[{"title": "A", "type": "T", "description": "da", "text_unit_ids": ["c1"], "frequency": 1, "degree": 1}],
        communities=[{"level": 0, "community_id": "C0", "parent": "C0", "entity_ids": ["A"]}],
    )
    # relationships needed by community_reports context
    pd.DataFrame([{"source": "A", "target": "A", "weight": 1.0, "description": ["d"], "text_unit_ids": ["c1"], "combined_degree": 2}]).to_parquet(f"{tmp_path}/relationships.parquet")
    step = repo.create_job(kb_id=1, type="full", specs=[StepSpec("community_reports", StepKind.UNIT_FANOUT)]).steps[0]
    worker = UnitWorker(repo=repo, adapter=_ReportFailAdapter("C0"), data_root=data_root)
    await worker.run_unit_fanout(step)  # must TERMINATE, not hang
    assert repo.get_step(step.id).status == StepStatus.PARTIALLY_FAILED
