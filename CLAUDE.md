...
### graphrag isolation seams — keep them clean
- `kb_platform/graph/adapter.py` defines the `GraphAdapter` Protocol (chunk / extract / summarize / report / cluster / finalize / embed). **graphrag imports are confined to `graphrag_adapter.py`, `graphrag_engine.py`, and `kb_platform/llm/`** — do not add graphrag/graphrag-llm imports elsewhere in the engine/api layers. `FakeGraphAdapter` (same file) is a deterministic, no-LLM implementation used by every engine test.
...
`CostCapturingCompletion` wraps graphrag-llm's `LLMCompletion`. The `UnitWorker` opens a `use_recorder()` contextvar per unit; concurrent asyncio tasks are isolated because each copies the context. Each `completion_async` records `response.usage` into the unit's `CostRecorder`; `to_json()` lands in `Unit.cost_json`, which `Repository._sum_cost` aggregates by step/model/job. Unknown models contribute tokens with `estimated_cost_usd=None` — cost **never** raises and a missing price latches the total to `None` ("unknown"), never zero. Multi-key round-robin + cross-profile failover live in the gateway (see LLM provider layer below); the old `LoadBalancingCompletion` is **deleted**. `CostCapturingCompletion` still wraps the completion, so this cost seam is unchanged.
...
assemble_kb_settings(kb, repo)   # profiles + decrypted keys + structured_output + content knobs → settings dict
                                  # llm.kb_profiles = ordered [primary, *fallbacks] (from kb.llm_fallback_profile_ids)
  → build_adapter_from_settings  # parses settings → ModelConfig(type="kb_native") + chunking/length/prompt params
    → build_default_adapter      # wires real graphrag chunker + extractors into a GraphRagAdapter
```
All LLM/embedding `ModelConfig`s are built with `type="kb_native"` + a `kb_profiles` extra, so graphrag-llm's factory returns the native client (see LLM provider layer below) — not litellm. `structured_output` (json_schema for community reports) follows the **LLM provider profile**, not the KB; DeepSeek needs `false` → `report_community_plain` path with lenient JSON parsing.

### LLM provider layer (`kb_native`) — litellm bypassed at runtime
`kb_platform/llm/` is a self-owned transport that replaces graphrag-llm/litellm on **every** chat-completion + embedding call (indexing extract/summarize/report/embed, query map-reduce/embed, rewriter). graphrag-llm exposes a first-class factory registry; at startup `bootstrap()` (called from `server.py` + `worker.py` entrypoints) runs `register_completion("kb_native", NativeCompletion)` + `register_embedding("kb_native", NativeEmbedding)` (idempotent; `ModelConfig.extra="allow"` carries the profile bundle). Every KB config sets `type="kb_native"`, so every `create_completion`/`create_embedding` — ours **and** graphrag's query factory's — returns the native client. No monkeypatch; `LiteLLMCompletion` is never instantiated on a hot path (`tests/llm/test_guard_no_litellm.py` stamps its `__init__` to raise).

Internals: `NativeCompletion`/`NativeEmbedding` (`client.py`/`embedding.py`, both graphrag-llm `LLMCompletion`/embedding subclasses) drive a `FailoverGateway` (`gateway.py`) over the ordered profile list with per-endpoint `CircuitBreaker`s (`circuit_breaker.py`, shared process-wide via `breaker_registry.py`). Provider SSE is parsed natively (`sse.py` → events `text_delta`/`tool_call_delta`/`usage`/`done`/`error`); per-provider URL/headers/auth in `request.py`; `httpx` is the only network client, drawn from a shared process pool (`http_client.py`, keyed by `ssl_verify`, closed on shutdown via `bootstrap.close_clients`). `_StreamFixWrapper` is deleted — `NativeCompletion`'s stream return is both async-iterable and awaitable (`_AwaitableAsyncIterator`), covering all four graphrag engines (basic does `async for chunk in X`, the others `async for chunk in await X`).

Failover: a retriable error (5xx/429/timeout/transport) on a profile trips its breaker (N consecutive → open → half-open after TTL) and advances to the next profile; non-retriable 401/403 also advances (different profile may have valid creds). A background `HealthProbe` (`health.py`) ticks each endpoint and feeds the shared breakers — it runs in the **server only** (the worker is `asyncio.run`-per-job with no persistent loop, so worker breakers are traffic-driven). `GET /llm/health` exposes per-endpoint breaker state + metrics (`metrics.py`: TTFT, failover detect/recover) — the registry is **per-process**, so `/llm/health` reflects the SERVER process only, not the worker's.
...
- **Proxy breaks localhost Ollama**: if `all_proxy`/`http_proxy`/`https_proxy` is set, the native httpx client (`kb_platform/llm/`) routes `localhost` through it and fails. Unset proxies or set `NO_PROXY=localhost,127.0.0.1`.
